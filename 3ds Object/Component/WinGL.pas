 ...
 {$IFDEF DESIGN}
  if (csDesigning in ComponentState) then begin
   with Canvas do begin
    ...
    TextOut(6,52,'GNU General Public License');
    TextOut(6,64,'Code by hackbard');
    TextOut(6,76,'Component Souce by Paul Toth');
    TextOut(6,88,'Coded for github.com');
    TextOut(6,100,'2026');
   end;
  end;
 {$ENDIF}
...
procedure TWinGL.TextMode;
 begin
  ...
  glMatrixMode(GL_PROJECTION); // changement de projection
   glPushMatrix; // sauver la projection en cours
   glLoadIdentity; // reset
   glOrtho(0,Width, 0,Height, -1, 1); // projection orthogonale
  glMatrixMode(GL_MODELVIEW); // mode drawing
   glPushMatrix; // sauver les infos
   glLoadIdentity; // reset
 end;

procedure TWinGL.GraphMode;
 begin
  glMatrixMode(GL_PROJECTION); // modifier projection
  glPopMatrix; // restaurer l'ancien
